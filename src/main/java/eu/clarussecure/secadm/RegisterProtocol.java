...
public class RegisterProtocol extends Command {
    ...
    public CommandReturn execute() throws CommandExecutionException {
        ...
        CommandReturn cr;
        if (success) {
            cr = new CommandReturn(0, "The Protocol '" + this.protocolName + "' was created with the schema '"
                    + this.protocolSchema + "'");
        } else {
    ...
    public boolean parseCommandArgs(String[] args) throws CommandParserException {
        ...
        if (!args[0].toLowerCase().equals("register_protocol"))
            throw new CommandParserException(
                    "Why a non-'register_protocol' command ended up in the 'register_protocol' part of the parser?");
